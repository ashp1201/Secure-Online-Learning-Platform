...
<body>
    ...
    let enrollmentMap = {}; // Map courseId to enrollmentId
        ...
        $.ajax({
            ...
            success: function (enrollments) {
                ...
                enrollments.forEach(e => {
                    enrollmentMap[e.courseId] = e.enrollmentId;
                });
                loadAvailableCourses();
            },
            error: function () {
    ...
    function displayCourses(courses, container, isEnrolledView) {
        ...
        courses.forEach(course => {
            const instructorName = course.instructorName || 'Unknown';
            ...
            <div class="course-card">
                <div class="course-header">
                    ...
                    ${isEnrolledView || isEnrolled ? '<span class="enrollment-status status-active">Enrolled</span>' : ''}
                </div>
    ...
    function renderVideoPlayer(courseId, contentPath) {
        if (!contentPath || contentPath.includes('fakepath\\Payment')) {
            return '<p style="color: #666; font-style: italic;">No video content uploaded yet.</p>';
        }
        const fileName = contentPath.split('\\').pop().split('/').pop();
        const enrollmentId = enrollmentMap[courseId];
        
        return `
        <div class="video-player">
            <video id="video-${courseId}" controls onloadedmetadata="initVideoTracking(${courseId}, ${enrollmentId})" ontimeupdate="trackVideoProgress(${courseId}, ${enrollmentId})">
                <source src="/Secure-Online-Learning-Platform/videos/${courseId}/${fileName}" type="video/mp4">
                Your browser does not support the video tag.
            </video>
        </div>`;
    }

    function initVideoTracking(courseId, enrollmentId) {
        const video = document.getElementById(`video-${courseId}`);
        video.dataset.enrollmentId = enrollmentId;
        video.dataset.lastUpdate = 0;
    }
    ...
    function updateProgress(enrollmentId, watchedPercent) {
        $.ajax({
            url: "/Secure-Online-Learning-Platform/progress/update",
            method: "POST",
            headers: { 
                "Authorization": "Bearer " + authToken,
            ...
            success: function() {
                console.log("Progress updated: " + watchedPercent.toFixed(2) + "%");
            },
            error: function(err) {
        ...
        $.ajax({
            ...
            success: function (enrollments) {
                ...
                if (!enrollments || enrollments.length === 0) {
                    enrolledList.html("<p>You haven't enrolled in any courses yet. <a href='#' onclick=\"showTab('browse-courses', event)\">Browse courses</a> to get started!</p>");
                    return;
                }
                ...
                const courses = enrollments.map(e => ({
                    courseId: e.courseId,
                    title: e.courseTitle,
                    instructorName: e.instructorName || 'Unknown',
                    category: e.category || 'N/A',
                    difficulty: e.difficulty || 'N/A',
        ...
        $.ajax({
            ...
            success: function (enrollments) {
                ...
                enrollments.forEach(enrollment => {
                    const progressHtml = `
                    <div class="course-card">
                        <div class="course-title">${escapeHtml(enrollment.courseTitle)}</div>
                        <div class="course-instructor" style="margin-bottom: 15px;">
                            <i class="fas fa-user"></i> ${escapeHtml(enrollment.instructorName || 'Unknown')}
                        </div>
                        ${renderProgressBar(enrollment.progressPercentage || 0)}
                    </div>`;
    ...
    function enrollCourse(courseId) {
        ...
        const enrollBtn = $(`#enroll-btn-${courseId}`);
        enrollBtn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Enrolling...');
        ...
        $.ajax({
            ...
            success: function (enrollment) {
                ...
                enrollmentMap[courseId] = enrollment.enrollmentId;
                loadAvailableCourses();
            },
            error: function (xhr) {
